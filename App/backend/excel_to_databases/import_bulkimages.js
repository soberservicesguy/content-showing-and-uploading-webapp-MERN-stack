...
const file_name = excel_file || '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_images.xlsx'
// const file_name = '/home/arsalan/Work_stuff/Full_stack_apps/REACT_APPS/Final_portfolio/content_app/backend/excel_to_databases/all_images.xlsx'
...
const save_parent_and_children_in_db = async (parent_children_rows_dict, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) =>{
			...
			if (related_children.length > 0){
				for (let k = 0; k < related_children.length; k++) {
					...
					for (let l = 0; l < child_rows.length; l++) {
						...
						const related_child = sheet_to_class_mapper(child_sheet, {...child_db_object_dict, image: image._id})
						...
						if (child_sheet === 'comments'){
							...
							image.comments.push(related_child._id)

		
						} else if (child_sheet === 'likes'){

							image.likes.push(related_child._id)

		
						} else if (child_sheet === 'users'){

							image.users.push(related_child._id)
...
const parent_children_detailed = (file_name, old_parent_child_relationship_data ,  parent_completely_detailed, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) =>{
	...
	// HERE, ACCESSED PARENT INDEX, NOW MATCH IT, LEARN BREAK AND CONTINUE
						...
						.then( (ans_for_above) => {
							if ( !all_results2.includes(ans_for_above) && ans_for_above !== undefined  ){
								...
								save_parent_and_children_in_db(ans_for_above, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res); 
							}
						})
...
const generate_parent_completely_detailed = (file_name, parent_child_relationship_data, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) => {
			...
			readXlsxFile( String(file_name), { sheet: String(parent_sheet) })
				.then(
				  ...
				  */
					...
					.then( parent_detailed => parent_children_detailed(file_name, parent_child_relationship_data, parent_detailed, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) )
...
const pull_parent_child_data_from_excel = (file_name, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) => {
	var all_collection = [];
...
// assigning children to parent_child_details dict 
	...
	.then( result => generate_parent_completely_detailed(file_name, result, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) )
	// .catch( (err) => console.log('ERROR IS ', err) )
}



const sheet_to_class = (file_name, user_id, folder_name, timestamp, attributes_with_paths, res) => {
	...
	.then(
		...
		.then ( (sheet_to_class_dict) => pull_parent_child_data_from_excel(file_name, sheet_to_class_dict, user_id, attributes_with_paths, folder_name, timestamp, res) )
...
if (excel_file){
	...
	// console.log('attempted to import images')
	// sheet_to_class(file_name)
}
