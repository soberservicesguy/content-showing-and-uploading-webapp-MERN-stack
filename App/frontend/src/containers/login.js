...
class LoginContainer extends Component {
	...
	login_and_get_jwt_token_and_privileges(){
		...
		let redirectToBlogpostsCallback  = () => this.setState(prev => ({...prev, redirectToBlogposts: true }))
	...
	delete_all_videos(){
		axios.get(utils.baseUrl + '/users/delete-all-videos',)
		.then(function (response) {
			if (response.data.success === true){
				...
				console.log('couldnt delete all videos')
			}
	...
	delete_all_likes(){
		axios.get(utils.baseUrl + '/users/delete-all-likes',)
		.then(function (response) {
			if (response.data.success === true){
				...
				console.log('couldnt delete all likes')
			}
	...
	delete_all_comments(){
		axios.get(utils.baseUrl + '/users/delete-all-comments',)
		.then(function (response) {
			if (response.data.success === true){
				...
				console.log('couldnt delete all comments')
			}
	...
	render() {
		...
		} else if ( this.state.redirectToBlogposts !== false ) {

			// switching it back to false
			this.setState(prev => ({...prev, redirectToBlogposts: (prev.redirectToBlogposts === false) ? true : false }))
				...
				<div style={styles.outerContainer}>
					...
					<div style={{
						...
						<div style={{...styles.formAndRounButtonContainer, marginLeft:50, backgroundColor: 'red', borderColor:'red'}}>
							<button 
								style={{...styles.roundButton, }}
								onClick={ () => this.delete_all_videos() }
							>
								Delete All Videos
							</button>
						</div>
						...
						<div style={{...styles.formAndRounButtonContainer, marginLeft:50, backgroundColor: 'red', borderColor:'red'}}>
							<button 
								style={{...styles.roundButton, }}
								onClick={ () => this.delete_all_comments() }
							>
								Delete All Comments
							</button>
						</div>
